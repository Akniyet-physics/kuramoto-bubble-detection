...
ax1.axvline(d_idx[crash], color="#EF5350", ls="--", label=f"Peak synchronization ({d_idx[crash].strftime('%Y-%m')})")
ax1.axhline(Rd[crash], color="#F9A825", ls=":", label=f"R_crash={Rd[crash]:.2f}")
ax1.set(title="Dotcoms 1995–2001", ylim=(0,1.05)); ax1.set_title(ax1.get_title(), color="white")
ax1.legend(fontsize=8, facecolor="#0D1B2A", labelcolor="white")

ax2.plot(a_idx, Ra, color="#F9A825", lw=1.8, label="R(t) AI")
ax2.axhline(Rd[crash], color="#EF5350", ls="--", label=f"Level of crash={Rd[crash]:.2f}")
ax2.axhline(Ra[-1], color="#66BB6A", ls=":", label=f"Now={Ra[-1]:.2f}")
ax2.set(title="AI-market 2023–2026", ylim=(0,1.05)); ax2.set_title(ax2.get_title(), color="white")
ax2.legend(fontsize=8, facecolor="#0D1B2A", labelcolor="white")

fig.suptitle(f"AI now: R={Ra[-1]:.2f}  |  Dotcom crash: R={Rd[crash]:.2f}  |  Difference: {Rd[crash]-Ra[-1]:.2f}", color="#F9A825")
